...
class ReactTelephoneInput extends React.Component {
  ...
  guessSelectedCountry(inputNumber) {
    const secondBestGuess =
      find(allCountries, { iso2: this.props.defaultCountry }) || this.props.onlyCountries[0]
    ...
    const onSetStateComplete = () => {
      if (isModernBrowser) {
        ...
        if (caretPosition > 0 && oldFormattedText.length >= formattedNumber.length) {
          this.numberInput.input.setSelectionRange(caretPosition, caretPosition)
        }
      }
  ...
  handleAutoselectListSelect = (selectedValue) => {
    ...
    const newSelectedCountry = this.guessSelectedCountry(selectedValue.substring(0, 6))
    formattedNumber = this.formatNumber(selectedValue, newSelectedCountry.format)
  ...
  render() {
    ...
    const { formattedNumber, showDropDown, selectedCountry, maxLength } = this.state
